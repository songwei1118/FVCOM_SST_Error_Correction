...
def load_ocean_data(file_path, normalize=True):
    ...
    print("      ERA5.U10")
    print("      ERA5.V10")
    print("      ERA5.short_wave")
    print("      ERA5.long_wave")
    print("      ERA5.air_pressure")
    print("      FVCOM.km")
    print("      FVCOM.u")
    print("      FVCOM.v")
    print("      ERA5.air_temperature")
    print("   FVCOM SST (FVCOM.temp): EXCLUDED")
    print(f"   Validity mask shape: {mask.shape}")
    print(f"   Longitude range: {lon.min():.2f} - {lon.max():.2f}")
    print(f"   Latitude range: {lat.min():.2f} - {lat.max():.2f}")
    print(f"   Number of time steps: {len(time)} days")
    print(f"   Time coverage: {time[0]} → {time[-1]}")
...
def split_data_sequential(X, Y, mask, time, train_ratio=0.7, val_ratio=0.15):
    ...
    print(f"Chronological dataset splitting:")
    print(f"   Training set: {len(train_idx)} time steps ({train_idx[0]} - {train_idx[-1]})")
    print(f"   Validation set: {len(val_idx)} time steps ({val_idx[0]} - {val_idx[-1]})")
    print(f"   Test set: {len(test_idx)} time steps ({test_idx[0]} - {test_idx[-1]})")

    print("\nCorresponding temporal coverage:")

    print("\nTraining set:")
    print("Start:", time[train_idx[0]])
    print("End:", time[train_idx[-1]])

    print("\nValidation set:")
    print("Start:", time[val_idx[0]])
    print("End:", time[val_idx[-1]])

    print("\nTest set:")
    print("Start:", time[test_idx[0]])
    print("End:", time[test_idx[-1]])
...
class Trainer:
    ...
    def train(self, train_loader, val_loader, num_epochs=50,
              save_path='No-FVCOM-SST-MSCT-PE-FVCOM-2010-2023-best-model.pth'):
        ...
        for epoch in range(num_epochs):
            ...
            if self.early_stop_counter >= self.early_stop_patience:
                print(f"\nEarly stopping was triggered at epoch {epoch + 1}")
                break

        print(f"Training completed. Best validation loss achieved: {self.best_val_loss:.6f}")
        return self.history
...
def main():
    ...
    print("=" * 60)
    print("MSCT-PE: FVCOM-SST-Excluded SST Reconstruction")
    print("=" * 60)

    output_dir = r'./No-FVCOM-SST-MSCT-PE-results_2010_2023'
    ...
    print("\nStep 2: Chronological dataset splitting")
    ...
    history = trainer.train(
        ...
        save_path=os.path.join(
            output_dir,
            'No-FVCOM-SST-MSCT-PE-FVCOM-2010-2023-best-model.pth'
        )
    )
