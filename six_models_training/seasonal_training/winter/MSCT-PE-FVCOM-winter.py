...
class Trainer:
    ...
    def train(self, train_loader, val_loader, num_epochs=50,
              save_path='CNN-Transformer-多尺度35绝对位置编码-动态学习率2010-2023 Winter-有气压_best_model.pth'):
...
def plot_temperature_comparison(model, test_dataset, means_X, stds_X, mean_Y, std_Y, device,
                                num_samples=60, output_path='CNN-Transformer-多尺度35绝对位置编码-动态学习率2010-2023 Winter-有气压_temperature_comparison.png'):
    model.eval()
    ...
    plt.title(f'Last {len(time_steps)} test samples temperature comparison (2010-2023 Winter)', fontsize=18, fontweight='bold')
...
def plot_mean_temperature_map_comparison(
        ...
        mean_Y, std_Y,
        device,
        output_path='CNN-Transformer-多尺度35绝对位置编码-动态学习率2010-2023 Winter-有气压_mean_temperature_map_comparison.png'):
    ...
    plt.suptitle(
        '2010-2023 Winter test-set mean SST spatial comparison',
        fontsize=16, fontweight='bold'
    )
...
def plot_scatter_prediction_vs_observation(
        ...
        device,
        last_n_samples=None,
        save_path='CNN-Transformer-多尺度35绝对位置编码-动态学习率2010-2023 Winter-有气压_scatter_pred_vs_obs_testset.png'):
    model.eval()

    total_len = len(test_dataset)
    if last_n_samples is not None:
        start_idx = max(0, total_len - last_n_samples)
        indices = range(start_idx, total_len)
        print(f"📊 Scatter plot uses last {len(indices)}  time samples "
              f"(index {start_idx}–{total_len - 1})")
    else:
        indices = range(total_len)
        print(f"📊 Scatter plot uses full test set ({total_len}  time samples)")
    ...
    with torch.no_grad():
        for idx in indices:
            ...
            valid_mask = mask_np == 1
            if np.any(valid_mask):
                y_obs_all.append(y_denorm[valid_mask].ravel())
                y_pred_all.append(pred_denorm[valid_mask].ravel())
    ...
    cb = plt.colorbar(hb)
    cb.set_label('log$_{10}$(Pixel counts)', fontsize=14)

    plt.xlabel('Predicted SST (°C)', fontsize=16)
    plt.ylabel('Observed SST (°C)', fontsize=16)

    plt.title(
        f'Predicted vs Observed SST (Winter test set)\n'
...
def main():
    ...
    history = trainer.train(
        ...
        num_epochs=100,
        save_path='MSCT-PE-FVCOM Winter_best_model.pth'
    )

    best_model_path = 'MSCT-PE-FVCOM Winter_best_model.pth'
