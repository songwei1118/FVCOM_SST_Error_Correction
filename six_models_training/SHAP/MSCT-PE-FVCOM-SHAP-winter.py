...
def batch_shap(explainer, dataset, batch_size=5, save_interval=None, save_dir="./shap_results"):
    ...
    for batch_idx in tqdm(range(0, len(dataset), batch_size), desc="Computing SHAP"):
        ...
        if save_interval and (batch_idx // batch_size + 1) % save_interval == 0:
            ...
            intermediate_path = os.path.join(save_dir, f"shap_intermediate_冬季_{batch_idx // batch_size + 1}.pkl")
...
def global_shap_analysis(shap_values, masks, feature_names, save_path=None):
    ...
    print("\n" + "=" * 60)
    print("Global Feature Importance Analysis - Winter")
    print("=" * 60)
...
def save_shap_results(shap_values, masks, test_indices=None, save_dir="./shap_results"):
    ...
    shap_path = os.path.join(save_dir, "shap_values_冬季.npy")
    mask_path = os.path.join(save_dir, "masks_冬季.npy")

    np.save(shap_path, shap_values)
    np.save(mask_path, masks)

    if test_indices is not None:
        idx_path = os.path.join(save_dir, "test_indices_冬季.npy")
    ...
    metadata_path = os.path.join(save_dir, "metadata_冬季.pkl")
    with open(metadata_path, 'wb') as f:
        pickle.dump(metadata, f)

    print(f"SHAP results saved to: {save_dir}")
    print(f"   - shap_values_冬季.npy: {shap_values.shape}")
    print(f"   - masks_冬季.npy: {masks.shape}")
    print(f"   - metadata_冬季.pkl: metadata saved")
...
def load_shap_results(save_dir="./shap_results"):
    ...
    print(f"Loading SHAP results (winter):")
...
def main():
    ...
    model_path = r"CNN-Transformer-多尺度35绝对位置编码-动态学习率2010-2023冬季-有气压_best_model.pth"
    save_dir = r"./shap_results_冬季_test"
    ...
    if not force_recompute and os.path.exists(shap_path):
        print("\n" + "=" * 60)
        print("Step 3: Load existing SHAP results (winter)")
        print("=" * 60)
        shap_values, masks, metadata = load_shap_results(save_dir)

        importance_path = os.path.join(save_dir, "feature_importance_冬季.csv")
        ...
        plt.title(f"全局SHAP特征重要性 - 冬季数据 (全测试集, {len(test_dataset)}个样本)")
        plt.tight_layout()

        importance_plot_path = os.path.join(save_dir, "shap_importance_冬季.png")
    ...
    print("\n" + "=" * 60)
    print("Step 3: Prepare SHAP background samples (winter)")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("Step 5: Compute SHAP values (full test set, winter)")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("Step 6: Save SHAP results (winter)")
    print("=" * 60)
    save_shap_results(shap_values, test_mask, save_dir=save_dir)

    print("\n" + "=" * 60)
    print("Step 7: Global feature importance analysis (winter)")
    print("=" * 60)
    importance_path = os.path.join(save_dir, "feature_importance_冬季.csv")
    df = global_shap_analysis(shap_values, test_mask, feature_names,
                              save_path=importance_path)
    ...
    print("\n" + "=" * 60)
    print("Full test set SHAP analysis for winter completed!")
