...
def batch_shap(explainer, dataset, batch_size=5, save_interval=None, save_dir="./shap_results"):
    ...
    for batch_idx in tqdm(range(0, len(dataset), batch_size), desc="Computing SHAP"):
        ...
        if save_interval and (batch_idx // batch_size + 1) % save_interval == 0:
            ...
            intermediate_path = os.path.join(save_dir, f"shap_intermediate_春季_{batch_idx // batch_size + 1}.pkl")
...
def global_shap_analysis(shap_values, masks, feature_names, save_path=None):
    ...
    print("\n" + "=" * 60)
    print("Global Feature Importance Analysis - Spring")
    print("=" * 60)
...
def save_shap_results(shap_values, masks, test_indices=None, save_dir="./shap_results"):
    ...
    shap_path = os.path.join(save_dir, "shap_values_春季.npy")
    mask_path = os.path.join(save_dir, "masks_春季.npy")

    np.save(shap_path, shap_values)
    np.save(mask_path, masks)

    if test_indices is not None:
        idx_path = os.path.join(save_dir, "test_indices_春季.npy")
    ...
    metadata_path = os.path.join(save_dir, "metadata_春季.pkl")
    ...
    print(f"   - metadata_春季.pkl: metadata saved")
...
def load_shap_results(save_dir="./shap_results"):
    ...
    print(f"Loading SHAP results (spring):")
...
def main():
    ...
    model_path = r"CNN-Transformer-多尺度35绝对位置编码-动态学习率2010-2023春季-有气压_best_model.pth"
    ...
    if not force_recompute and os.path.exists(shap_path):
        print("\n" + "=" * 60)
        print("Step 3: Load existing SHAP results (spring)")
        print("=" * 60)
        shap_values, masks, metadata = load_shap_results(save_dir)

        importance_path = os.path.join(save_dir, "feature_importance_春季.csv")
        ...
        plt.title(f"全局SHAP特征重要性 - 春季数据 (全测试集, {len(test_dataset)}个样本)")
        plt.tight_layout()

        importance_plot_path = os.path.join(save_dir, "shap_importance_春季.png")
    ...
    print("\n" + "=" * 60)
    print("Step 3: Prepare SHAP background samples (spring)")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("Step 5: Compute SHAP values (full test set, spring)")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("Step 6: Save SHAP results (spring)")
    print("=" * 60)
    save_shap_results(shap_values, test_mask, save_dir=save_dir)

    print("\n" + "=" * 60)
    print("Step 7: Global feature importance analysis (spring)")
    print("=" * 60)
    importance_path = os.path.join(save_dir, "feature_importance_春季.csv")
    df = global_shap_analysis(shap_values, test_mask, feature_names,
                              save_path=importance_path)
    ...
    print("\n" + "=" * 60)
    print("Full test set SHAP analysis for spring completed!")
