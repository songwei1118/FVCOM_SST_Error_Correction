...
class Trainer:
    ...
    def train(self, train_loader, val_loader, num_epochs=100,
              save_path='多尺度CNN-Transformer-2010-2023_best_model.pth'):
...
def plot_rmse_maps(rmse_model, rmse_fvcom,
                   final_path_model='MSCT-FVCOM-RMSE_model_crop.png',
                   final_path_fvcom='MSCT-FVCOM-RMSE_fvcom_crop.png'):
    ...
    draw_and_crop(rmse_model, 'MSCT-FVCOM RMSE ', final_path_model)
...
def plot_mean_temperature_map_comparison(
        ...
        mean_Y, std_Y,
        device,
        save_path_sat='消融实验-MSCT-SST_satellite_mean.png',
        save_path_fvcom='消融实验-MSCT-SST_fvcom_mean.png',
        save_path_model='消融实验-MSCT-SST_model_mean.png'):
    ...
    draw_single(pred_mean, 'MSCT-FVCOM SST', save_path_model)
...
def plot_bias_maps(bias_model, bias_fvcom,
    ...
    draw(bias_model, "MSCT Bias", save_path_model)


def main():

    output_dir = r'./消融实验-MSCT-results_2010_2023'
    ...
    print("\nStep 4: Creating CNN-Transformer model")
    ...
    best_model_path = '多尺度CNN-Transformer-2010-2023_best_model.pth'
    ...
    save_path = os.path.join(output_dir, "消融实验-MSCT-evaluation_metrics.csv")
    ...
    np.save(os.path.join(output_dir, "消融实验-MSCT_bias_model.npy"), bias_model)
    np.save(os.path.join(output_dir, "消融实验-MSCT_bias_fvcom.npy"), bias_fvcom)

    np.save(os.path.join(output_dir, "消融实验-MSCT_rmse_model.npy"), rmse_model)
    np.save(os.path.join(output_dir, "消融实验-MSCT_rmse_fvcom.npy"), rmse_fvcom)

    np.save(os.path.join(output_dir, "消融实验-MSCT_improvement.npy"), improvement)

    print("Spatial statistics saved (for post-processing plots)")

    plot_rmse_maps(rmse_model, rmse_fvcom,
                   final_path_model=os.path.join(output_dir, '消融实验-MSCT-model_rmse_final.png'),
                   final_path_fvcom=os.path.join(output_dir, '消融实验-MSCT-fvcom_rmse_final.png'))
    ...
    plot_bias_maps(
        bias_model,
        bias_fvcom,
        save_path_model=os.path.join(output_dir, '消融实验-MSCT-bias_model.png'),
        save_path_fvcom=os.path.join(output_dir, '消融实验-MSCT-bias_fvcom.png')
    )
    ...
    plot_mean_temperature_map_comparison(
        ...
        save_path_sat=os.path.join(output_dir, '消融实验-MSCT-SST_satellite_mean.png'),
        save_path_fvcom=os.path.join(output_dir, '消融实验-MSCT-SST_fvcom_mean.png'),
        save_path_model=os.path.join(output_dir, '消融实验-MSCT-SST_model_mean.png')
    )
