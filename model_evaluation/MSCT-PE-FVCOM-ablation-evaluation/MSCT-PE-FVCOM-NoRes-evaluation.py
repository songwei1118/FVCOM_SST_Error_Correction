...
class Trainer:
    ...
    def train(self, train_loader, val_loader, num_epochs=100,
              save_path='MSCT-PE-消融实验去掉残差_best_model.pth'):
...
def plot_rmse_maps(rmse_model, rmse_fvcom,
                   final_path_model='消融实验-MSCT-PE-去掉残差-RMSE_model_crop.png',
                   final_path_fvcom='消融实验-MSCT-PE-去掉残差-RMSE_fvcom_crop.png'):
    ...
    draw_and_crop(rmse_model, 'MSCT-PE-FVCOM-no-residual RMSE ', final_path_model)
...
def plot_mean_temperature_map_comparison(
        ...
        mean_Y, std_Y,
        device,
        save_path_sat='消融实验-MSCT-PE-去掉残差-SST_satellite_mean.png',
        save_path_fvcom='消融实验-MSCT-PE-去掉残差-SST_fvcom_mean.png',
        save_path_model='消融实验-MSCT-PE-去掉残差-SST_model_mean.png'):
    ...
    draw_single(pred_mean, 'MSCT-PE-FVCOM-no-residual SST', save_path_model)
...
def plot_bias_maps(bias_model, bias_fvcom,
    ...
    draw(bias_model, "MSCT-PE-FVCOM-no-residual Bias", save_path_model)

def main():


    output_dir = r'./消融实验-MSCT-PE-去掉残差-results_2010_2023'
    ...
    best_model_path = 'MSCT-PE-消融实验去掉残差_best_model.pth'
    ...
    save_path = os.path.join(output_dir, "消融实验-MSCT-PE-去掉残差-evaluation_metrics.csv")
    ...
    np.save(os.path.join(output_dir, "消融实验-MSCT-PE-去掉残差_bias_model.npy"), bias_model)
    np.save(os.path.join(output_dir, "消融实验-MSCT-PE-去掉残差_bias_fvcom.npy"), bias_fvcom)

    np.save(os.path.join(output_dir, "消融实验-MSCT-PE-去掉残差_rmse_model.npy"), rmse_model)
    np.save(os.path.join(output_dir, "消融实验-MSCT-PE-去掉残差_rmse_fvcom.npy"), rmse_fvcom)

    np.save(os.path.join(output_dir, "消融实验-MSCT-PE-去掉残差_improvement.npy"), improvement)

    print("Spatial statistics saved (for post-processing plots)")

    plot_rmse_maps(rmse_model, rmse_fvcom,
                   final_path_model=os.path.join(output_dir, '消融实验-MSCT-PE-去掉残差-model_rmse_final.png'),
                   final_path_fvcom=os.path.join(output_dir, '消融实验-MSCT-PE-去掉残差-fvcom_rmse_final.png'))
    ...
    plot_bias_maps(
        bias_model,
        bias_fvcom,
        save_path_model=os.path.join(output_dir, '消融实验-MSCT-PE-去掉残差-bias_model.png'),
        save_path_fvcom=os.path.join(output_dir, '消融实验-MSCT-PE-去掉残差-bias_fvcom.png')
    )
    ...
    plot_mean_temperature_map_comparison(
        ...
        save_path_sat=os.path.join(output_dir, '消融实验-MSCT-PE-去掉残差-SST_satellite_mean.png'),
        save_path_fvcom=os.path.join(output_dir, '消融实验-MSCT-PE-去掉残差-SST_fvcom_mean.png'),
        save_path_model=os.path.join(output_dir, '消融实验-MSCT-PE-去掉残差-SST_model_mean.png')
    )
