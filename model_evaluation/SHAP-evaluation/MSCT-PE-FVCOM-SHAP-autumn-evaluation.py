...
def batch_shap(explainer, dataset, batch_size=5, save_interval=None):
    ...
    for batch_idx in tqdm(range(0, len(dataset), batch_size), desc="Computing SHAP"):
        ...
        if save_interval and (batch_idx // batch_size + 1) % save_interval == 0:
            ...
            with open(f"shap_intermediate_秋季_{batch_idx // batch_size + 1}.pkl", 'wb') as f:
...
def save_shap_results(shap_values, masks, test_indices=None, save_dir="./shap_results_秋季"):
    os.makedirs(save_dir, exist_ok=True)

    np.save(os.path.join(save_dir, "shap_values_秋季.npy"), shap_values)
    np.save(os.path.join(save_dir, "masks_秋季.npy"), masks)

    if test_indices is not None:
        np.save(os.path.join(save_dir, "test_indices_秋季.npy"), test_indices)
    ...
    with open(os.path.join(save_dir, "metadata_秋季.pkl"), 'wb') as f:
...
def load_shap_results(save_dir="./shap_results_秋季"):
    shap_values = np.load(os.path.join(save_dir, "shap_values_秋季.npy"))
    masks = np.load(os.path.join(save_dir, "masks_秋季.npy"))

    if shap_values.ndim == 5 and shap_values.shape[-1] == 1:
        shap_values = np.squeeze(shap_values, axis=-1)
        print("Automatically removed last SHAP dimension (...,1)")

    with open(os.path.join(save_dir, "metadata_秋季.pkl"), 'rb') as f:
...
def main():
    ...
    df = global_shap_analysis(shap_values, test_mask, feature_names,
                              save_path=os.path.join(save_dir, "feature_importance_秋季.csv"))

    plot_feature_importance(
        df,
        save_path=os.path.join(save_dir, "shap_importance_bar_秋季.png")
    )

    print("Autumn data evaluation complete")
