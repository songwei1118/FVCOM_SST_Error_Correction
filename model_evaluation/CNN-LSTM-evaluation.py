...
def split_data_sequential(X, Y, mask, train_ratio=0.7, val_ratio=0.15):
    ...
    print(f"   Train: first {len(train_idx)} time steps (index: {train_idx[0]} - {train_idx[-1]})")
    print(f"   Val: middle {len(val_idx)} time steps (index: {val_idx[0]} - {val_idx[-1]})")
    print(f"   Test: last {len(test_idx)} time steps (index: {test_idx[0]} - {test_idx[-1]})")
...
def train_model(
    ...
    print("\nStart training (Early Stopping enabled)")
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if no_improve_epochs >= early_stop_patience:
            ...
            print(f"Validation loss did not improve for {early_stop_patience} consecutive epochs")
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title('ConvLSTM Train / Val Loss (Early Stopping)')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
...
def plot_rmse_maps(rmse_model, rmse_fvcom,
                   final_path_model='CNN-LSTM-FVCOM-RMSE_model_crop.png',
                   final_path_fvcom='CNN-LSTM-FVCOM-RMSE_fvcom_crop.png'):
    ...
    draw_and_crop(rmse_model, 'CNN-LSTM-FVCOM RMSE ', final_path_model)
...
def plot_mean_temperature_map_comparison(
        ...
        mean_Y, std_Y,
        device,
        save_path_sat='CNN-LSTM-FVCOM-SST_satellite_mean.png',
        save_path_fvcom='CNN-LSTM-FVCOM-SST_fvcom_mean.png',
        save_path_model='CNN-LSTM-FVCOM-SST_model_mean.png'):
    ...
    draw_single(pred_mean, 'CNN-LSTM-FVCOM SST', save_path_model)
...
def plot_bias_maps(bias_model, bias_fvcom,
    ...
    draw(bias_model, "CNN-LSTM-FVCOM Bias", save_path_model)
...
def main():
    ...
    train, val, test, test_idx = split_data_sequential(X, Y, mask)
    ...
    save_path = os.path.join(output_dir, "CNN-LSTM-evaluation_metrics.csv")
    results_df.to_csv(save_path, index=False)

    print(f"RMSE and MAE saved to: {save_path}")

    print("\nStep 6: compute spatial RMSE maps")
    ...
    np.save(os.path.join(output_dir, "CNN-LSTM-FVCOM_bias_model.npy"), bias_model)
    np.save(os.path.join(output_dir, "CNN-LSTM-FVCOM_bias_fvcom.npy"), bias_fvcom)

    np.save(os.path.join(output_dir, "CNN-LSTM-FVCOM_rmse_model.npy"), rmse_model)
    np.save(os.path.join(output_dir, "CNN-LSTM-FVCOM_rmse_fvcom.npy"), rmse_fvcom)

    np.save(os.path.join(output_dir, "CNN-LSTM-FVCOM_improvement.npy"), improvement)

    print("Spatial statistics saved (for post-processing plots)")

    plot_rmse_maps(rmse_model, rmse_fvcom,
                   final_path_model=os.path.join(output_dir, 'CNN-LSTM-FVCOM-model_rmse_final.png'),
                   final_path_fvcom=os.path.join(output_dir, 'CNN-LSTM-FVCOM-fvcom_rmse_final.png'))

    plot_bias_maps(
        bias_model,
        bias_fvcom,
        save_path_model=os.path.join(output_dir, 'CNN-LSTM-FVCOM-bias_model.png'),
        save_path_fvcom=os.path.join(output_dir, 'CNN-LSTM-FVCOM-bias_fvcom.png')
    )

    print("\nStep 8: plot spatial mean temperature comparison")
    plot_mean_temperature_map_comparison(
        ...
        save_path_sat=os.path.join(output_dir, 'CNN-LSTM-FVCOM-SST_satellite_mean.png'),
        save_path_fvcom=os.path.join(output_dir, 'CNN-LSTM-FVCOM-SST_fvcom_mean.png'),
        save_path_model=os.path.join(output_dir, 'CNN-LSTM-FVCOM-SST_model_mean.png')
    )
