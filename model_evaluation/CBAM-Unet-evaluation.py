...
def train_model(model, train_dataset, val_dataset, device, num_epochs=100, batch_size=4, lr=5e-4,
                save_dir="./results_CBAMunet_year_2010_2023", patience=10):
    ...
    best_model_path = os.path.join(save_dir, "best_resunet_model_cbamunet.pth")
    epochs_no_improve = 0
    ...
    plt.savefig(os.path.join(save_dir, "loss_curve_cbamunet_year_2010_2023.png"), dpi=150)
    plt.close()
...
def evaluate_model(model, test_ds, means, stds, Y_mean, Y_std, device,
                   save_path="evaluation_results_cbamUNET_year_2010_2023.txt"):
    model.eval()
...
def plot_bias_maps(bias_model, bias_fvcom,
    ...
    draw(bias_model, "CBAM-Unet-FVCOM Bias", save_path_model)


def plot_rmse_maps(rmse_model, rmse_fvcom,
                   final_path_model='CBAM-Unet-FVCOM-RMSE_model_crop.png',
                   final_path_fvcom='CBAM-Unet-FVCOM-RMSE_fvcom_crop.png'):
    ...
    draw_and_crop(rmse_model, 'CBAM-Unet-FVCOM RMSE ', final_path_model)
...
def plot_mean_temperature_map_comparison(
        ...
        mean_Y, std_Y,
        device,
        save_path_sat='CBAM-Unet-FVCOM-SST_satellite_mean.png',
        save_path_fvcom='CBAM-Unet-FVCOM-SST_fvcom_mean.png',
        save_path_model='CBAM-Unet-FVCOM-SST_model_mean.png'):
    ...
    draw_single(pred_mean, 'CBAM-Unet-FVCOM SST', save_path_model)


def main():
    set_seed(46)
    DATA_PATH = r'D:\CNN\春季数据处理\output_2010_2023_full_year_strict\data_with_mask_2010_2023_full_year.mat'
    output_dir = r'./CBAM-Unet-results_2010_2023'
    ...
    best_model_path = r'D:\CNN\CBAM_Unet模型\results_UNet_CBAM_year_2010_2023\best_resunet_model_cbamunet.pth'
    ...
    save_path = os.path.join(output_dir, "CBAM-Unet-evaluation_metrics.csv")
    ...
    np.save(os.path.join(output_dir, "CBAM-Unet-FVCOM_bias_model.npy"), bias_model)
    np.save(os.path.join(output_dir, "CBAM-Unet-FVCOM_bias_fvcom.npy"), bias_fvcom)

    np.save(os.path.join(output_dir, "CBAM-Unet-FVCOM_rmse_model.npy"), rmse_model)
    np.save(os.path.join(output_dir, "CBAM-Unet-FVCOM_rmse_fvcom.npy"), rmse_fvcom)

    np.save(os.path.join(output_dir, "CBAM-Unet-FVCOM_improvement.npy"), improvement)

    print("Spatial statistics saved (for post-processing plots)")

    plot_rmse_maps(rmse_model, rmse_fvcom,
                   final_path_model=os.path.join(output_dir, 'CBAM-Unet-FVCOM-model_rmse_final.png'),
                   final_path_fvcom=os.path.join(output_dir, 'CBAM-Unet-FVCOM-fvcom_rmse_final.png'))

    plot_bias_maps(
        bias_model,
        bias_fvcom,
        save_path_model=os.path.join(output_dir, 'CBAM-Unet-FVCOM-bias_model.png'),
        save_path_fvcom=os.path.join(output_dir, 'CBAM-Unet-FVCOM-bias_fvcom.png')
    )
    ...
    plot_mean_temperature_map_comparison(
        ...
        save_path_sat=os.path.join(output_dir, 'CBAM-Unet-FVCOM-SST_satellite_mean.png'),
        save_path_fvcom=os.path.join(output_dir, 'CBAM-Unet-FVCOM-SST_fvcom_mean.png'),
        save_path_model=os.path.join(output_dir, 'CBAM-Unet-FVCOM-SST_model_mean.png')
    )
