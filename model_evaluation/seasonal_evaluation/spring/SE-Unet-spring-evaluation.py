...
def train_model(model, train_dataset, val_dataset, device, num_epochs=100, batch_size=2, lr=5e-4,
    ...
    best_model_path = os.path.join(save_dir, "best_resunet_model_seunet_春季.pth")
    epochs_no_improve = 0
    ...
    plt.savefig(os.path.join(save_dir, "loss_curve_seunet_year_2010_2023_春季.png"), dpi=150)
    plt.close()
...
def evaluate_model(model, test_ds, means, stds, Y_mean, Y_std, device,
                   save_path="evaluation_results_seUNET_year_2010_2023_春季.txt"):
    model.eval()
...
def plot_temperature_comparison_simple(model, test_ds, test_idx, means, stds, Y_mean, Y_std, device,
    ...
    plt.plot(time_steps, adjusted_temps, color='r', linewidth=2.5, label='SEUnet Adjustment')

    plt.xlabel('Test Time Steps (Last 60 Samples)', fontsize=14)
    plt.ylabel('Temperature (°C)', fontsize=14)
    plt.title(f'最后{len(time_steps)}个测试样本的温度对比 (2010-2023年全年)', fontsize=15)
    ...
    plt.tight_layout()
    plt.savefig('temperature_comparison_last_60_samples_year_SEUnet_2010_2023_春季.png',
                dpi=300, bbox_inches='tight')
    plt.show()

    print(
 f"Temperature comparison plot of the last {len(time_steps)} test samples saved: temperature_comparison_last_60_samples_year_SEUnet_2010_2023_春季.png")


def plot_mean_temperature_comparison(model, test_ds, lon, lat, means, stds, Y_mean, Y_std, device,
                                     output_path='mean_temperature_comparison_SEUnet_year_2010_2023_春季.png'):
    model.eval()
    loader = DataLoader(test_ds, batch_size=1)
    ...
    print(f"After correction (SEUnet) temperature range: {np.nanmin(pred_mean):.3f}°C ~ {np.nanmax(pred_mean):.3f}°C")
    ...
    plt.suptitle('2010-2023年全年海表温度订正效果对比 - 区域全天数平均',
...
def main():
    ...
    MODEL_PATH = r'D:\CNN\SE_Unet模型\results_UNet_SE_spring_2010_2023\best_resunet_model_seunet_春季.pth'
    SAVE_DIR = "./results_UNet_SE_spring_2010_2023"
    ...
    evaluation_path_csv = os.path.join(SAVE_DIR, "evaluation_results_seunet_year_2010_2023_spring.csv")
    ...
    results = evaluate_model(
        ...
        Y_std,
        device,
        save_path=os.path.join(SAVE_DIR, "evaluation_results_seunet_year_2010_2023_spring.txt")
    )
    ...
    plot_temperature_comparison_simple(model, test_ds, test_idx, means, stds, Y_mean, Y_std, device, num_samples=100)
