...
def evaluate_full_testset_metrics_spring(
        model,
        test_ds,
        means,
        stds,
...
def plot_temperature_comparison_simple_spring(model, test_ds, test_idx, means, stds, Y_mean, Y_std, device, last_n=60):
    model.eval()
    ...
    plt.tight_layout()
    plt.savefig('temperature_comparison_CNNLSTM_spring_2010_2023.png',
                dpi=300, bbox_inches='tight')
    plt.show()

    print(f"Three-curve temperature comparison figure saved: temperature_comparison_CNNLSTM_spring_2010_2023.png (last {len(sst_temps)} samples shown)")


def plot_mean_temperature_comparison_spring(model, test_ds, lon, lat, means, stds, Y_mean, Y_std, device,
                                     output_path='mean_temperature_comparison_CNNLSTM_spring_2010_2023.png'):
...
def main_spring():
    ...
    model_path = 'best_CNNLSTM_model_spring_2010_2023.pth'
    ...
    print("=" * 60)
    eval_results = evaluate_full_testset_metrics_spring(model, test_ds, means, stds, Y_mean, Y_std, device)

    results_df = pd.DataFrame([eval_results])

    csv_path = "CNNLSTM_春季_test_metrics_2010_2023.csv"
    results_df.to_csv(csv_path, index=False)

    print(f"\nEvaluation metrics saved to: {csv_path}")
    print(results_df)

    plot_temperature_comparison_simple_spring(model, test_ds, test_idx, means, stds, Y_mean, Y_std, device, last_n=60)

    plot_mean_temperature_comparison_spring(model, test_ds, lon, lat, means, stds, Y_mean, Y_std, device,
                                     output_path='mean_temperature_comparison_CNNLSTM_spring_2010_2023.png')
