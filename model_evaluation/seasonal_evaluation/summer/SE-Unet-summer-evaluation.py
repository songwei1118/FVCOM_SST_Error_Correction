...
def train_model(model, train_dataset, val_dataset, device, num_epochs=100, batch_size=4, lr=5e-4,
    ...
    plt.ylabel("Loss")
    plt.legend()
    plt.title("Training & Validation Loss (2010-2023 Summer)")
    plt.savefig(os.path.join(save_dir, "loss_curve_seunet_summer_2010_2023.png"), dpi=150)
    plt.close()
...
def evaluate_model(model, test_ds, means, stds, Y_mean, Y_std, device,
                   save_path="evaluation_results_seUNET_summer_2010_2023.txt"):
    model.eval()
...
def plot_temperature_comparison_simple(model, test_ds, test_idx, means, stds, Y_mean, Y_std, device,
    ...
    plt.tight_layout()
    plt.savefig('temperature_comparison_last_60_samples_summer_SEUnet_2010_2023.png',
                dpi=300, bbox_inches='tight')
    plt.show()

    print(
 f"Temperature comparison plot of the last {len(time_steps)} test samples saved: temperature_comparison_last_60_samples_summer_SEUnet_2010_2023.png")


def plot_mean_temperature_comparison(model, test_ds, lon, lat, means, stds, Y_mean, Y_std, device,
                                     output_path='mean_temperature_comparison_SEUnet_summer_2010_2023.png'):
    model.eval()
    loader = DataLoader(test_ds, batch_size=1)
...
def main():
    ...
    MODEL_PATH = r'D:\CNN\SE_Unet模型\results_UNet_SE_summer_2010_2023\best_resunet_model_seunet_夏季 .pth'
    SAVE_DIR = "./results_UNet_SE_summer_2010_2023"
    ...
    evaluation_path_csv = os.path.join(SAVE_DIR, "evaluation_results_seunet_year_2010_2023_summer.csv")
    ...
    results = evaluate_model(
        ...
        Y_std,
        device,
        save_path=os.path.join(SAVE_DIR, "evaluation_results_seunet_year_2010_2023_summer.txt")
    )
