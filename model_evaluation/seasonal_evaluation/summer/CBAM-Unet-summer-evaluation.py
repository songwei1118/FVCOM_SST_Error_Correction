...
def train_model(model, train_dataset, val_dataset, device, num_epochs=100, batch_size=2, lr=5e-4,
                save_dir="./results_CBAMunet_year_2010_2023_夏季", patience=10):
    ...
    best_model_path = os.path.join(save_dir, "best_resunet_model_cbamunet_夏季.pth")
    epochs_no_improve = 0
    ...
    plt.ylabel("Loss")
    plt.legend()
    plt.title("Training & Validation Loss (2010-2023_Summer)")
    plt.savefig(os.path.join(save_dir, "loss_curve_cbamunet_year_2010_2023_夏季.png"), dpi=150)
    plt.close()
...
def evaluate_model(model, test_ds, means, stds, Y_mean, Y_std, device,
                   save_path="evaluation_results_cbamUNET_year_2010_2023_夏季.txt"):
    model.eval()
    ...
    with open(save_path, "w", encoding="utf-8") as f:
        f.write("========== Model Evaluation Results (2010-2023_Summer) ==========\n")
...
def plot_temperature_comparison_simple(model, test_ds, test_idx, means, stds, Y_mean, Y_std, device,
    ...
    plt.title(f'Temperature Comparison of the Last {len(time_steps)} Test Samples (2010-2023_Summer)', fontsize=15)
    ...
    plt.tight_layout()
    plt.savefig('temperature_comparison_last_60_samples_year_CBAMUnet_2010_2023_夏季.png',
                dpi=300, bbox_inches='tight')
    plt.show()

    print(
        f"Temperature comparison plot for the last {len(time_steps)} test samples saved: "
        f"temperature_comparison_last_60_samples_year_CBAMUnet_2010_2023_夏季.png")


def plot_mean_temperature_comparison(model, test_ds, lon, lat, means, stds, Y_mean, Y_std, device,
                                     output_path='mean_temperature_comparison_CBAMUnet_year_2010_2023_夏季.png'):
    model.eval()
    loader = DataLoader(test_ds, batch_size=1)
    ...
    plt.suptitle('SST Correction Effect Comparison 2010-2023 - Regional All-Day Mean_Summer',
...
def main():
    ...
    MODEL_PATH = r'D:\CNN\CBAM_Unet模型\results_UNet_CBAM_year_2010_2023_夏季\best_resunet_model_cbamunet_夏季.pth'
    SAVE_DIR = "./results_UNet_CBAM_year_2010_2023_夏季"
    ...
    evaluation_path_csv = os.path.join(SAVE_DIR, "evaluation_results_CBAMunet_year_2010_2023_summer.csv")
    ...
    results = evaluate_model(
        ...
        Y_std,
        device,
        save_path=os.path.join(SAVE_DIR, "evaluation_results_CBAMunet_year_2010_2023_summer.txt")
    )
