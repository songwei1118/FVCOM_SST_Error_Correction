...
def load_data(file_path=r'D:\CNN\夏季数据处理\output_2010_2023_summer\data_with_mask_2010_2023_summer.mat'):
...
def train_model(model, train_dataset, val_dataset, device, num_epochs=100, batch_size=4, lr=5e-4,
                save_dir="./results_SAunet_year_2010_2023_summer", patience=10):
    ...
    best_model_path = os.path.join(save_dir, "best_resunet_model_saunet_summer.pth")
    epochs_no_improve = 0
    ...
    plt.ylabel("Loss")
    plt.legend()
    plt.title("Training & Validation Loss (2010-2023 Year Summer)")
    plt.savefig(os.path.join(save_dir, "loss_curve_saunet_year_2010_2023_summer.png"), dpi=150)
    plt.close()
...
def evaluate_model(model, test_ds, means, stds, Y_mean, Y_std, device,
                   save_path="evaluation_results_saUNET_year_2010_2023_summer.txt"):
...
def plot_temperature_comparison_simple(model, test_ds, test_idx, means, stds, Y_mean, Y_std, device,
    ...
    if n_samples < num_samples:
        print(f"Warning: test set has only {n_samples} samples, fewer than the requested {num_samples}")
    ...
    plt.plot(time_steps, adjusted_temps, color='r', linewidth=2.5, label='SAUnet Adjustment')

    plt.xlabel('Test Time Steps (Last 60 Samples)', fontsize=14)
    plt.ylabel('Temperature (°C)', fontsize=14)
    plt.title(f'最后{len(time_steps)}个测试样本的温度对比 (2010-2023年夏季)', fontsize=15)
    ...
    plt.tight_layout()
    plt.savefig('temperature_comparison_last_60_samples_year_SAUnet_2010_2023_summer.png',
                dpi=300, bbox_inches='tight')
    plt.show()

    print(
        f"The last {len(time_steps)} test samples temperature comparison plot saved: temperature_comparison_last_60_samples_year_SAUnet_2010_2023_summer.png")


def plot_mean_temperature_comparison(model, test_ds, lon, lat, means, stds, Y_mean, Y_std, device,
                                     output_path='mean_temperature_comparison_SAUnet_year_2010_2023_summer.png'):
    model.eval()
    loader = DataLoader(test_ds, batch_size=1)
    ...
    print(f"Satellite SST mean temperature range: {np.nanmin(sst_mean):.3f}°C ~ {np.nanmax(sst_mean):.3f}°C")
    print(f"Before correction (FVCOM) temperature range: {np.nanmin(fvcom_mean):.3f}°C ~ {np.nanmax(fvcom_mean):.3f}°C")
    print(f"After correction (SAUnet) temperature range: {np.nanmin(pred_mean):.3f}°C ~ {np.nanmax(pred_mean):.3f}°C")
    ...
    plt.suptitle('2010-2023年夏季海表温度订正效果对比 - 区域全天数平均',
    ...
    print("\nStatistics:")
    print(f"Mean satellite observed temperature: {np.nanmean(sst_mean):.3f} ± {np.nanstd(sst_mean):.3f}°C")
    print(f"Mean temperature before correction: {np.nanmean(fvcom_mean):.3f} ± {np.nanstd(fvcom_mean):.3f}°C")
    print(f"Mean temperature after correction: {np.nanmean(pred_mean):.3f} ± {np.nanstd(pred_mean):.3f}°C")
    print(f"Mean temperature change: {np.nanmean(diff_mean):.3f}°C")
...
def main():
    ...
    DATA_PATH = r'D:\CNN\夏季数据处理\output_2010_2023_summer\data_with_mask_2010_2023_summer.mat'
    MODEL_PATH = r'./results_UNet_SA_year_2010_2023_summer/best_resunet_model_saunet_summer.pth'
    SAVE_DIR = "./results_UNet_SA_year_2010_2023_summer"
    ...
    evaluation_path_csv = os.path.join(SAVE_DIR, "evaluation_results_saunet_year_2010_2023_summer.csv")
    ...
    results = evaluate_model(
        ...
        Y_std,
        device,
        save_path=os.path.join(SAVE_DIR, "evaluation_results_saunet_year_2010_2023_summer.txt")
    )
