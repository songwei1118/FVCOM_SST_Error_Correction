...
def train_model(model, train_dataset, val_dataset, device, num_epochs=100, batch_size=4, lr=5e-4,
                save_dir="./results_SAunet_year_2010_2023_winter", patience=10):
    ...
    best_model_path = os.path.join(save_dir, "best_resunet_model_saunet_winter.pth")
    epochs_no_improve = 0
    ...
    plt.ylabel("Loss")
    plt.legend()
    plt.title("Training & Validation Loss (2010-2023 Year Winter)")
    plt.savefig(os.path.join(save_dir, "loss_curve_saunet_year_2010_2023_winter.png"), dpi=150)
    plt.close()
...
def evaluate_model(model, test_ds, means, stds, Y_mean, Y_std, device,
                   save_path="evaluation_results_saUNET_year_2010_2023_winter.txt"):
...
def plot_temperature_comparison_simple(model, test_ds, test_idx, means, stds, Y_mean, Y_std, device,
    ...
    plt.title(f'最后{len(time_steps)}个测试样本的温度对比 (2010-2023年冬季)', fontsize=15)
    ...
    plt.tight_layout()
    plt.savefig('temperature_comparison_last_60_samples_year_SAUnet_2010_2023_winter.png',
                dpi=300, bbox_inches='tight')
    plt.show()

    print(
        f"The last {len(time_steps)} test samples temperature comparison plot saved: temperature_comparison_last_60_samples_year_SAUnet_2010_2023_winter.png")


def plot_mean_temperature_comparison(model, test_ds, lon, lat, means, stds, Y_mean, Y_std, device,
                                     output_path='mean_temperature_comparison_SAUnet_year_2010_2023_winter.png'):
    model.eval()
    loader = DataLoader(test_ds, batch_size=1)
    ...
    plt.suptitle('2010-2023年冬季海表温度订正效果对比 - 区域全天数平均',
...
def main():
    ...
    DATA_PATH = r'D:\CNN\冬季数据处理\output_2010_2023_winter\data_with_mask_2010_2023_winter.mat'
    MODEL_PATH = r'./results_UNet_SA_year_2010_2023_winter/best_resunet_model_saunet_winter.pth'
    SAVE_DIR = "./results_UNet_SA_year_2010_2023_winter"
    ...
    evaluation_path_csv = os.path.join(SAVE_DIR, "evaluation_results_saunet_year_2010_2023_winter.csv")
    ...
    results = evaluate_model(
        ...
        Y_std,
        device,
        save_path=os.path.join(SAVE_DIR, "evaluation_results_saunet_year_2010_2023_winter.txt")
    )
