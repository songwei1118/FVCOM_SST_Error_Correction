...
class OceanDataset_autumn(Dataset):
...
def load_data_autumn(file_path=r'D:\CNN\秋季数据处理\output_2010_2023_autumn\data_with_mask_2010_2023_autumn.mat'):
...
def split_data_sequential_autumn(X, Y, mask, train_ratio=0.7, val_ratio=0.15):
    N = len(X)
...
class ResBlock_autumn(nn.Module):
...
class CNNResLSTM_autumn(nn.Module):
...
def train_model_autumn(
    ...
    best_model_path = os.path.join(
        save_dir, 'best_CNNLSTM_model_autumn_2010_2023.pth'
    )
    last_model_path = os.path.join(
        save_dir, 'last_CNNLSTM_model_autumn_2010_2023.pth'
    )
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig('loss_curve_CNNLSTM_autumn_2010_2023.png', dpi=300)
    plt.show()

    return train_losses, val_losses, best_model_path


def evaluate_full_testset_metrics_autumn(
        model,
        test_ds,
        means,
        stds,
...
def plot_temperature_comparison_simple_autumn(model, test_ds, test_idx, means, stds, Y_mean, Y_std, device, last_n=60):
    model.eval()
    ...
    plt.tight_layout()
    plt.savefig('temperature_comparison_CNNLSTM_autumn_2010_2023.png',
                dpi=300, bbox_inches='tight')
    plt.show()

    print(f"Three-curve temperature comparison figure saved: temperature_comparison_CNNLSTM_autumn_2010_2023.png (last {len(sst_temps)} samples shown)")


def plot_mean_temperature_comparison_autumn(model, test_ds, lon, lat, means, stds, Y_mean, Y_std, device,
                                     output_path='mean_temperature_comparison_CNNLSTM_autumn_2010_2023.png'):
...
def plot_scatter_prediction_vs_obs_autumn(
    model,
    test_ds,
    Y_mean,
    Y_std,
    ...
    last_n_samples=None,
    max_points=None,
    save_path='scatter_pred_vs_obs_CNNLSTM_autumn_2010_2023.png'
):
    ...
    if last_n_samples is not None:
        start_idx = max(0, n_total - last_n_samples)
        indices = range(start_idx, n_total)
        print(f"Using last {n_total - start_idx} test samples")
    else:
        indices = range(n_total)
        print("Using full test dataset")

    y_obs_all, y_pred_all = [], []
    ...
    with torch.no_grad():
        for idx in indices:
            ...
            valid = (m == 1)
            if np.any(valid):
                y_obs_all.append(y[valid].ravel())
                y_pred_all.append(pred[valid].ravel())

    y_obs = np.concatenate(y_obs_all)
    y_pred = np.concatenate(y_pred_all)

    print(f"Total valid pixels: {len(y_obs):,}")

    if max_points is not None and len(y_obs) > max_points:
        idx = np.random.choice(len(y_obs), max_points, replace=False)
        y_obs = y_obs[idx]
        y_pred = y_pred[idx]
        print(f"Randomly sampled to {max_points:,} pixels")
    ...
    cb = plt.colorbar(hb)
    cb.set_label('log$_{10}$(Pixel counts)', fontsize=11)

    plt.xlabel('Predicted SST (°C)', fontsize=12)
    plt.ylabel('Observed SST (°C)', fontsize=12)

    title_suffix = (
        f'Last {n_total - start_idx} Test Samples'
        if last_n_samples is not None
        else 'Full Test Set'
    )

    plt.title(
        f'Predicted vs Observed SST CNNLSTM ({title_suffix})\n'
        f'RMSE = {rmse:.3f} °C,  R$^2$ = {r2:.3f}',
        fontsize=12
    )
...
def main_autumn():
    set_seed(46)
    X, Y, mask, lon, lat, time, means, stds, Y_mean, Y_std = load_data_autumn(
        r'D:\CNN\秋季数据处理\output_2010_2023_autumn\data_with_mask_2010_2023_autumn.mat')
    train, val, test, test_idx = split_data_sequential_autumn(X, Y, mask)
    train_ds = OceanDataset_autumn(*train)
    val_ds = OceanDataset_autumn(*val)
    test_ds = OceanDataset_autumn(*test)

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print("Using device:", device)

    model = CNNResLSTM_autumn(in_channels=10, input_height=X.shape[2], input_width=X.shape[3])

    model_path = 'best_CNNLSTM_model_autumn_2010_2023.pth'
    ...
    print("=" * 60)
    eval_results = evaluate_full_testset_metrics_autumn(model, test_ds, means, stds, Y_mean, Y_std, device)

    results_df = pd.DataFrame([eval_results])

    csv_path = "CNNLSTM_秋季_test_metrics_2010_2023.csv"
    results_df.to_csv(csv_path, index=False)

    print(f"\nEvaluation metrics saved to: {csv_path}")
    print(results_df)

    plot_temperature_comparison_simple_autumn(model, test_ds, test_idx, means, stds, Y_mean, Y_std, device, last_n=60)

    plot_mean_temperature_comparison_autumn(model, test_ds, lon, lat, means, stds, Y_mean, Y_std, device,
                                     output_path='mean_temperature_comparison_CNNLSTM_autumn_2010_2023.png')
