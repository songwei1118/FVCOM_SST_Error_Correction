...
def evaluate_mhw_model(
    ...
    print(f"SE-Unet-FVCOM RMSE   : {model_rmse:.4f} °C")
    print(f"RMSE Improvement     : {rmse_improvement:.2f} %")

    print("-" * 65)

    print(f"FVCOM MAE            : {fvcom_mae:.4f} °C")
    print(f"SE-UnetFVCOM MAE     : {model_mae:.4f} °C")
    ...
    result_df = pd.DataFrame({
        ...
        "SE-Unet_FVCOM_RMSE_C": [model_rmse],
        "RMSE_Improvement_percent": [rmse_improvement],

        "FVCOM_MAE_C": [fvcom_mae],
        "SE-Unet_FVCOM_MAE_C": [model_mae],
        "MAE_Improvement_percent": [mae_improvement]
    })

    save_path = (
        r"D:\CNN\SE-Unet模型"
        r"\SE-Unet_FVCOM_MHW_RMSE_MAE.csv"
    )
