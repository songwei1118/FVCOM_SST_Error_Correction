...
def load_ocean_data(file_path, normalize=True):
    ...
    if normalize:
        ...
        print(f"Target normalization: mean={mean_Y:.4f}, std={std_Y:.4f}")
    else:
...
class Trainer:
    ...
    def train(self, train_loader, val_loader, num_epochs=50,
        ...
        print(f"Using LR scheduler: ReduceLROnPlateau")
...
def plot_rmse_maps(rmse_model, rmse_fvcom,
                   final_path_model='MSCT-PE-FVCOM-RMSE_model_crop.png',
                   final_path_fvcom='MSCT-PE-FVCOM-RMSE_fvcom_crop.png'):
    ...
    def draw_and_crop(data, title, final_path):
        ...
        cbar.set_label('RMSE (C)', fontsize=16)
    ...
    draw_and_crop(rmse_model, 'MSCT-PE-FVCOM RMSE', final_path_model)
...
def plot_mean_temperature_map_comparison(
        ...
        mean_Y, std_Y,
        device,
        save_path_sat='MSCT-PE-FVCOM-SST_satellite_mean.png',
        save_path_fvcom='MSCT-PE-FVCOM-SST_fvcom_mean.png',
        save_path_model='MSCT-PE-FVCOM-SST_model_mean.png'):
    ...
    draw_single(pred_mean, 'MSCT-PE-FVCOM SST', save_path_model)
...
def plot_bias_maps(bias_model, bias_fvcom,
    ...
    draw(bias_model, "MSCT-PE-FVCOM Bias", save_path_model)


def main():
    print("=" * 60)
    print("Ocean SST prediction model - load best model (2010-2023)")
    print("=" * 60)

    output_dir = r'./MSCT-PE-FVCOM-results_2010_2023'
    ...
    print("\nStep X: Test-set time series save")
    ...
    save_path = os.path.join(output_dir, "MSCT-PE-FVCOM-evaluation_metrics.csv")
    ...
    np.save(os.path.join(output_dir, "MSCT-PE-FVCOM_bias_model.npy"), bias_model)
    np.save(os.path.join(output_dir, "MSCT-PE-FVCOM_bias_fvcom.npy"), bias_fvcom)

    np.save(os.path.join(output_dir, "MSCT-PE-FVCOM_rmse_model.npy"), rmse_model)
    np.save(os.path.join(output_dir, "MSCT-PE-FVCOM_rmse_fvcom.npy"), rmse_fvcom)

    np.save(os.path.join(output_dir, "MSCT-PE-FVCOM_improvement.npy"), improvement)

    print("Spatial statistics saved.")

    plot_rmse_maps(rmse_model, rmse_fvcom,
                   final_path_model=os.path.join(output_dir, 'MSCT-PE-FVCOM-model_rmse_final.png'),
                   final_path_fvcom=os.path.join(output_dir, 'MSCT-PE-FVCOM-fvcom_rmse_final.png'))

    print("\nStep 7: Plot bias maps")

    plot_bias_maps(
        bias_model,
        bias_fvcom,
        save_path_model=os.path.join(output_dir, 'MSCT-PE-FVCOM-bias_model.png'),
        save_path_fvcom=os.path.join(output_dir, 'MSCT-PE-FVCOM-bias_fvcom.png')
    )

    print("\nStep 8: Plot mean SST comparison")
    plot_mean_temperature_map_comparison(
        ...
        std_Y=data['std_Y'],
        device=device,
        save_path_sat=os.path.join(output_dir, 'MSCT-PE-FVCOM-SST_satellite_mean.png'),
        save_path_fvcom=os.path.join(output_dir, 'MSCT-PE-FVCOM-SST_fvcom_mean.png'),
        save_path_model=os.path.join(output_dir, 'MSCT-PE-FVCOM-SST_model_mean.png')
    )
